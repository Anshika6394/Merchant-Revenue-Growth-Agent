...
def test_revenue_metrics_are_database_derived(client):
    ...
    data = response.json()
    assert data["gross_revenue"] == 450590
    assert data["net_revenue"] == 438800.5
    assert data["successful_payment_revenue"] == 450590
    assert data["order_count"] == 410
    assert data["transaction_count"] == 730
    assert data["aov"] == 1099
...
def test_payments_include_configurable_recoverable_revenue(client):
    ...
    assert recoverable["failed_value"] == 359730
    assert recoverable["recovery_assumption"] == 0.4
    assert recoverable["estimated_recoverable_revenue"] == 143892
...
def test_overview_checkout_customer_subscription_refund_and_trend_metrics(client):
    ...
    assert data["checkout"]["abandoned_value"] == 54915
    assert data["customers"]["repeat_customers"] == 180
    assert data["customers"]["win_back_signals"] == 65
    assert data["subscriptions"] == {"active": 65, "cancelled": 8, "past_due": 17, "failed_recurring_payments": 34, "subscription_revenue": 78335, "retention_risk": 17}
    assert data["refunds"]["refund_count"] == 18
    assert data["refunds"]["refund_value"] == 11789.5
